...
export const CustomizerWizard: React.FC<CustomizerWizardProps> = ({
  ...
  // Re-sync whenever initialProduct prop changes from external interactions
  useEffect(() => {
    if (initialProduct) {
      const match = candles.find((c) => c.id === initialProduct.id) || initialProduct;
      handleSelectCandle(match);
    }
  }, [initialProduct?.id]);
  ...
  const activeLayers: Layer2D[] = useMemo(() => {
    ...
    // Filter out Santuario de Lavanda layers if on Oso
    if (selectedCandle?.id === "vela-custom-1789491779825" || selectedCandle?.name?.toLowerCase().includes("oso")) {
      raw = raw.filter((l) => !l.imageUrl?.includes("WhatsApp_Image_2026-09-09_at_4.46.35_PM"));
    }

    // Ensure Jirafa layer has valid Supabase URL and colorable: true
    if (selectedCandle?.id === "vela-custom-1789490882289" || selectedCandle?.name?.toLowerCase().includes("jirafa")) {
      raw = raw.map((l) => ({
        ...l,
  ...
  }, [selectedCandle?.layers2D, selectedCandle?.id, selectedCandle?.name, selectedFigure?.layers2D, selectedVessel?.layers2D]);
  ...
  const handleSelectCandle = (candle: CandleProduct) => {
    ...
    // Match sculpture if candle is associated with one
    if (candle.sculptureType) {
      const matchedFig = sculptures.find(
        (s) =>
          s.id === candle.sculptureType ||
          s.sculptureType === candle.sculptureType ||
          s.id.toLowerCase().includes(candle.sculptureType.toLowerCase())
      );
      if (matchedFig) {
      ...
      const matchedFig = sculptures.find(
        (s) =>
          s.name.toLowerCase().includes(candle.name.toLowerCase()) ||
          candle.name.toLowerCase().includes(s.name.toLowerCase())
      );
      if (matchedFig) {
        setSelectedFigure(matchedFig);
      }
    ...
    if (candle.customLabelSubtitle || candle.subtitle) {
      setLabelSubtitle(candle.customLabelSubtitle || candle.subtitle);
    }

    // Initialize/sync customer layer colors directly from this candle's layers2D
    const effectiveCandleLayers =
      candle.layers2D && candle.layers2D.length > 0 ? candle.layers2D : [];
    if (effectiveCandleLayers.length > 0) {
      const defaults: Record<string, string> = {};
      effectiveCandleLayers.forEach((l) => {
        if (l.colorable && l.defaultColorHex) {
          defaults[l.id] = l.defaultColorHex;
        }
      });
  ...
  // Price Calculation:
  // Base price is taken directly from the selected candle as set in the administrator.
  // The figure, wax, and relief are already part of the candle product and should NOT incur an additional figurePrice surcharge.
  const basePrice =
    selectedCandle && selectedCandle.price !== undefined && selectedCandle.price !== null && !isNaN(Number(selectedCandle.price))
      ? Number(selectedCandle.price)
      : 22.0;
  ...
  const handleAddToCart = () => {
    ...
    const candleProduct: CandleProduct = {
      ...
      sculptureType: selectedCandle?.sculptureType || selectedFigure?.id,
      layers2D: selectedCandle?.layers2D && selectedCandle.layers2D.length > 0
        ? selectedCandle.layers2D
        : selectedFigure?.layers2D && selectedFigure.layers2D.length > 0
        ? selectedFigure.layers2D
        : [],
    ...
    const cartLayers = selectedCandle?.layers2D && selectedCandle.layers2D.length > 0
      ? selectedCandle.layers2D
      : selectedFigure?.layers2D && selectedFigure.layers2D.length > 0
      ? selectedFigure.layers2D
      : [];

    const customLayersDetail = cartLayers
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#E5E0DA] shadow-xs space-y-6 min-h-[480px] flex flex-col justify-between">
              {/* STEP 1: MODELO DEL CATÁLOGO & RELIEVE 2D */}
                ...
                <div className="space-y-6 animate-fade-in">
                  <div className="border-b border-[#E5E0DA]/80 pb-4">
                    ...
                    <h3 className="font-serif text-xl sm:text-2xl font-bold text-[#423D33]">
                      Elección de Modelo de Vela
                    </h3>
                    <p className="text-xs text-[#8C7A6B] mt-1">
                      Selecciona una de las velas publicadas en el catálogo para personalizar sus capas de cera, frasco, aroma y dedicatoria.
                    </p>
                  </div>

                  {/* Velas Publicadas del Catálogo */}
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-[11px] font-bold text-[#8C7A6B] uppercase tracking-wider">
                      <span>Velas Disponibles en Catálogo ({candles.length}):</span>
                      <span className="text-[10px] font-normal normal-case italic">
                        Carga directa de capas 2D y relieves guardados
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 max-h-[420px] overflow-y-auto pr-1">
                      {candles.map((candle) => {
                        ...
                        const layerCount = candle.layers2D?.length || 0;
                        return (
                          <div
                            key={candle.id}
                          ...
                          >
                            <div className="flex items-start gap-3">
                              <div className="w-16 h-16 rounded-xl bg-[#F4EFEA] border border-[#E5E0DA] overflow-hidden shrink-0 flex items-center justify-center p-1">
                                {Boolean(candle.image && candle.image.trim()) ? (
                                  ...
                                  <span className="text-base font-bold text-[#8C7A6B]">
                              ...
                              <div className="flex-1 min-w-0 space-y-0.5">
                                ...
                                <p className="text-[10px] text-[#8C7A6B] truncate">
                                  {candle.subtitle || "Vela botánica en relieve"}
                                </p>
                                <div className="pt-1 flex items-center gap-1.5 flex-wrap">
                                  <span className="text-[9px] font-bold text-[#8C7A6B] bg-[#8C7A6B]/10 px-1.5 py-0.5 rounded-md flex items-center gap-1">
                                    ...
                                    <span>{layerCount} {layerCount === 1 ? "Capa 2D" : "Capas 2D"}</span>
                                  </span>
                                  <span className="text-[9px] text-[#35522e] bg-[#608058]/10 px-1.5 py-0.5 rounded-md font-medium">
                                    {candle.waxType || "Soja"}
                                  </span>
                                  {candle.vesselName && (
                                    <span className="text-[9px] text-[#423D33]/70 bg-stone-100 px-1.5 py-0.5 rounded-md truncate max-w-[110px]">
                                      {candle.vesselName}
                                    </span>
                                  )}
                                </div>
                            ...
                            <div className="mt-2.5 pt-2 border-t border-[#E5E0DA]/60 flex items-center justify-between text-[10px] text-[#8C7A6B]">
                              <span className="truncate max-w-[170px] italic">
                                {candle.tagline || candle.description?.slice(0, 42) || "Vela oficial del catálogo"}
                              </span>
                              {isSelected ? (
                                ...
                                <span className="text-[#8C7A6B]/70 group-hover:text-[#423D33] font-medium">
                                  Personalizar modelo →
                                </span>
                              )}
                            </div>
                    ...
                    <div className="p-3.5 rounded-2xl bg-[#F7F5F0] border border-[#D9C5B2]/60 shadow-xs flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-medium text-[#423D33]">
                          Diseño activo: <strong>{selectedCandle?.name || "Vela"}</strong> con <strong>{activeLayers.filter((l) => l.colorable).length} {activeLayers.filter((l) => l.colorable).length === 1 ? 'capa con color personalizable' : 'capas con colores personalizables'}</strong>.
                        </span>
                      </div>
