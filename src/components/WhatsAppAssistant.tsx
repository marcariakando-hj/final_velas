...
export const WhatsAppAssistant: React.FC<WhatsAppAssistantProps> = ({
  ...
  const processQuery = (
    ...
    if (isRelaxQuery) {
      ...
      return {
        text: `🌿 [RECOMENDACIÓN DESTACADA PARA RELAJACIÓN & CALMA]\n` +
          ...
          `• Precio: $${relaxCandle.price}.00 USD\n` +
    ...
    if (isGiftQuery) {
      ...
      return {
        text: `🎁 [RECOMENDACIÓN DESTACADA PARA REGALO ESPECIAL]\n` +
          ...
          `• Precio: $${giftCandle.price}.00 USD\n` +
    ...
    if (isWarmHomeQuery) {
      ...
      return {
        text: `🕯️ [RECOMENDACIÓN DESTACADA • HOGAR CÁLIDO & ACOGEDOR]\n` +
          ...
          `• Precio: $${warmCandle.price}.00 USD\n` +
    ...
    if (isFreshZenQuery) {
      ...
      return {
        text: `🌱 [RECOMENDACIÓN DESTACADA • FRESCURA & VITALIDAD ZEN]\n` +
          ...
          `• Precio: $${freshCandle.price}.00 USD\n` +
    ...
    if (matchedCandle) {
      return {
        text: `🕯️ [DETALLES DE: ${matchedCandle.name}]\n` +
          `• Precio: $${matchedCandle.price}.00 USD\n` +
    ...
    if (matchedTerms.length > 0) {
      ...
      if (scentMatches.length > 0) {
        const matchesList = scentMatches
          .map(
            (c) =>
              `• **${c.name}** ($${c.price}.00) — ${c.subtitle} [${c.inStock ? "✅ Disponible" : "❌ Agotada"}]`
          )
          .join("\n");
    ...
    if (
      ...
      const summaryList = candidates
        .map(
          (c) =>
            `• **${c.name}**: $${c.price}.00 USD — ${c.inStock ? "✅ En Stock" : "❌ Agotada"} (${c.subtitle})`
        )
        .join("\n");
    ...
    return {
      text: `No encontré esa consulta exacta en el catálogo, pero te recomiendo explorar nuestra Pieza de Autor destacada del momento: **${topFeatured.name}** ($${topFeatured.price}.00 USD • ${topFeatured.subtitle}).\n\n¿Deseas consultarme sobre aromas, regalos o contactar a un asesor humano? 🌸`,
      recommendedCandle: topFeatured,
      recommendHuman: true,
    };
  };
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      // 1. Query Gemini AI Assistant endpoint
      ...
      if (res.ok) {
        ...
        if (data.isAi && data.text) {
          // Identify any mentioned candles to attach interactive cards
          const matched = candles.find((c) =>
      ...
      console.warn("Falling back to local knowledge engine:", err);
    }

    // 2. Fallback to grounded local knowledge engine
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-white/80 flex items-center gap-1">
                  <span>En línea</span> • <span>Sommelier IA & Destacados</span>
                </p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-3.5 space-y-3 bg-[#efeae2]/60">
            {messages.map((msg) => (
              ...
              >
                ...
                >
                  {msg.isAi && (
                    <span className="inline-flex items-center gap-1 text-[9px] font-bold text-[#8C7A6B] bg-[#FAF7F2] px-1.5 py-0.5 rounded-md border border-[#E5E0DA] mb-1.5">
                      <Sparkles className="w-2.5 h-2.5 text-[#D98B68]" /> Respuesta Inteligente
                    </span>
                  )}
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] font-bold text-[#8C7A6B] mt-0.5">
                            ${msg.recommendedCandle.price}.00 USD
                          </p>
                        </div>
                      </div>
            ...
            {isLoading && (
              <div className="flex items-start gap-2 animate-fade-in">
                <div className="bg-white border border-[#E5E0DA] rounded-2xl rounded-tl-xs p-3 shadow-xs text-xs text-[#8C7A6B] flex items-center gap-2">
                  <Sparkles className="w-3.5 h-3.5 text-[#D98B68] animate-spin" />
                  <span>El sommelier botánico está redactando...</span>
                </div>
              </div>
            )}
