...
export const Unified2DModelManager: React.FC<Unified2DModelManagerProps> = ({
  ...
  const processFileForLayer = async (file: File, layerId: string) => {
    ...
    try {
      ...
      // 2. PERSISTENCIA EN EL SERVIDOR (/api/upload)
      try {
        ...
        if (res.ok) {
          ...
          if (data.url) {
            ...
            setUploadStatus({
              type: "success",
              message: `✓ Capa "${targetLayer.name}" guardada y sincronizada correctamente.`,
            });
            return;
          }
        }
      ...
      // 3. FALLBACK: SUPABASE STORAGE (si está configurado)
      if (isSupabaseConfigured()) {
        try {
          ...
          setUploadStatus({
            type: "success",
            message: `✓ Capa "${targetLayer.name}" subida y guardada con éxito.`,
          });
          return;
        } catch (cloudErr: any) {
          console.warn("Supabase Storage fallback:", cloudErr);
          setUploadStatus({
            type: "info",
  ...
  const [isBgDragOver, setIsBgDragOver] = useState<boolean>(false);

  const processBackgroundFile = async (file: File) => {
    if (!file) return;

    if (!file.type.startsWith("image/") && !file.name.match(/\.(png|jpe?g|webp|svg)$/i)) {
      setUploadStatus({
        type: "error",
        message: "Formato no válido para el fondo. Por favor sube un archivo PNG, JPG o WebP.",
      });
      return;
    }
    ...
    setUploadStatus({
      type: "info",
      message: `Cargando imagen de fondo "${file.name}"...`,
    });

    try {
      // 1. Vista previa local inmediata para renderizado instantáneo
      const immediateDataUrl = await new Promise<string>((resolve, reject) => {
        const reader = new FileReader();
      ...
      // Actualizar inmediatamente estado local para visualización sin espera
      updateResources({
        ...localResources,
        backgroundImage: immediateDataUrl,
        base2DImage: "",
      });

      // 2. Compresión optimizada conservando canal alfa con fallback
      let optimizedDataUrl = immediateDataUrl;
      try {
        ...
        if (compressed) optimizedDataUrl = compressed;
      } catch (compErr) {
        console.warn("Compresión de imagen de fondo fallback a DataURL nativa:", compErr);
      }

      let finalUrl = optimizedDataUrl;
      let isServerSaved = false;

      // 3. Persistencia en servidor local /api/upload mediante JSON
      try {
        const serverRes = await fetch("/api/upload", {
          ...
          body: JSON.stringify({
            image: optimizedDataUrl,
            name: `bg_${file.name.replace(/[^a-zA-Z0-9]/g, "_")}`,
          }),
        });
        if (serverRes.ok) {
        ...
        console.warn("Servidor /api/upload no disponible para fondo:", err);
      }

      // 4. Fallback a Supabase Storage si está configurado
      if (!isServerSaved && isSupabaseConfigured()) {
        try {
          const safeName = file.name.replace(/[^a-zA-Z0-9.-]/g, "_");
          const cloudUrl = await uploadFileToSupabaseStorage(
            file,
            `backgrounds/${Date.now()}_${safeName}`
          );
          if (cloudUrl) {
            finalUrl = cloudUrl;
            isServerSaved = true;
          ...
          console.warn("Fallo Supabase Storage para fondo:", cloudErr);
        }
      }


      // 5. Aplicar URL definitiva persistente
      updateResources({
        ...localResources,
      ...
      setUploadStatus({
        type: "success",
        message: isServerSaved
          ? "✓ Imagen de fondo subida y persistida correctamente."
          : "✓ Imagen de fondo aplicada y visible en la vista previa interactiva.",
      });
    } catch (err) {
      console.error("Error al procesar imagen de fondo:", err);
      setUploadStatus({
        type: "error",
        message: "Error al procesar la imagen de fondo. Verifica que sea una imagen válida.",
      });
    } finally {
  ...
  const handleRemoveBackground = () => {
    ...
    setUploadStatus({
      type: "info",
      message: "Imagen de fondo eliminada. La vista previa ahora tiene fondo limpio.",
    });
    if (bgFileInputRef.current) bgFileInputRef.current.value = "";
  };
  ...
  return (
    ...
    >
      ...
      {/* Hidden File Input for uploading Background Image (PNG or JPG) */}
      <input
        ref={bgFileInputRef}
        type="file"
      ...
      {/* NUEVO CAMPO: IMAGEN DE FONDO DE LA VISTA PREVIA */}
      <div
        onDragOver={handleBgDragOver}
        onDragLeave={handleBgDragLeave}
        onDrop={handleBgDrop}
        className={`bg-white p-4 sm:p-5 rounded-2xl border transition-all shadow-2xs space-y-3 ${
          isBgDragOver
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <span className="p-1.5 rounded-lg bg-[#FAF7F2] text-[#8C7A6B] border border-[#E5E0DA]">
              <ImageIcon className="w-4 h-4" />
            </span>
            <div>
              <div className="flex items-center gap-2">
                <h4 className="font-serif font-bold text-sm text-[#423D33]">
                  Imagen de Fondo de la Vista Previa
                </h4>
                {Boolean(resources.backgroundImage && resources.backgroundImage.trim()) ? (
              ...
              <p className="text-xs text-[#8C7A6B]">
                Sube una imagen (PNG o JPG) que servirá de fondo detrás de todas las capas 2D tanto en este panel como en la personalización del cliente. Si no se sube ninguna, se mostrará un fondo limpio sin ninguna imagen predeterminada.
              </p>
            </div>
          </div>
          {Boolean(resources.backgroundImage && resources.backgroundImage.trim()) && (
            <button
              ...
              className="self-start sm:self-auto text-xs text-red-600 hover:text-red-700 font-semibold flex items-center gap-1 transition-colors px-2.5 py-1 rounded-lg hover:bg-red-50 cursor-pointer"
            >
        ...
        {Boolean(resources.backgroundImage && resources.backgroundImage.trim()) ? (
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-3.5 bg-[#FAF7F2] rounded-xl border border-[#E5E0DA]">
            {/* Thumbnail preview */}
            <div className="relative w-20 h-20 sm:w-24 sm:h-24 bg-white rounded-lg border border-[#E5E0DA] overflow-hidden flex items-center justify-center shrink-0 shadow-2xs">
              <img
                src={resources.backgroundImage}
                alt="Imagen de Fondo de la Vista Previa"
                referrerPolicy="no-referrer"
                className="w-full h-full object-contain p-1"
                style={{ opacity: resources.backgroundOpacity ?? 1 }}
              />
            </div>
            ...
            <div className="flex-1 w-full space-y-3">
              <div className="flex items-center gap-2 flex-wrap">
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-lg bg-white border border-[#D9C5B2] hover:border-[#8C7A6B] text-xs font-bold text-[#423D33] flex items-center gap-1.5 cursor-pointer shadow-2xs transition-colors hover:bg-stone-50"
                >
                  <Upload className="w-3.5 h-3.5 text-[#8C7A6B]" />
                  <span>{isUploadingBg ? "Procesando..." : "Reemplazar Imagen de Fondo"}</span>
                </button>
                <button
                  type="button"
                  onClick={handleRemoveBackground}
                  className="px-3 py-1.5 rounded-lg bg-white border border-red-200 hover:border-red-300 text-xs font-medium text-red-600 hover:bg-red-50 flex items-center gap-1.5 cursor-pointer shadow-2xs transition-colors"
                >
                  <Trash2 className="w-3.5 h-3.5" />
                  <span>Quitar</span>
                </button>
                <span className="text-[11px] text-[#8C7A6B]">
                  O arrastra un archivo PNG, JPG o WebP sobre esta caja
                </span>
              </div>

              {/* Opacity slider */}
              <div className="space-y-1">
                <div className="flex items-center justify-between text-xs font-semibold text-[#423D33]">
                  ...
                  <span className="font-mono text-[11px] text-[#8C7A6B]">
                    {Math.round((resources.backgroundOpacity ?? 1) * 100)}%
                  </span>
                </div>
                <input
                  type="range"
                  min="0"
                  max="100"
                  value={Math.round((resources.backgroundOpacity ?? 1) * 100)}
                  onChange={(e) => handleBackgroundOpacityChange(Number(e.target.value) / 100)}
                  className="w-full accent-[#8C7A6B] cursor-pointer h-1.5 bg-[#E5E0DA] rounded-lg"
                />
              </div>
            </div>
          </div>
          ...
          <div
            onClick={() => bgFileInputRef.current?.click()}
            className={`p-5 border-2 border-dashed rounded-xl text-center cursor-pointer transition-all space-y-2 ${
              isBgDragOver
                ? "border-[#8C7A6B] bg-[#F4EFEA]"
                : "border-[#D9C5B2] hover:border-[#8C7A6B] bg-[#FAF7F2]/60 hover:bg-[#FAF7F2]"
            }`}
          >
            <div className="w-10 h-10 rounded-full bg-white border border-[#E5E0DA] flex items-center justify-center mx-auto text-[#8C7A6B] shadow-2xs">
              <Upload className="w-5 h-5" />
            </div>
            <div>
              <p className="text-xs font-bold text-[#423D33]">
                {isUploadingBg
                  ? "Procesando imagen de fondo..."
                  : isBgDragOver
                  ? "Suelta la imagen aquí para asignarla como fondo"
                  : "Haz clic o arrastra aquí para subir la Imagen de Fondo (PNG o JPG)"}
              </p>
              <p className="text-[11px] text-[#8C7A6B] mt-0.5">
                Opcional: Si no subes ninguna imagen, el fondo se mantendrá 100% limpio (blanco o transparente).
              </p>
            </div>
          </div>
        )}
