...
export const CollaboratorsSection: React.FC<CollaboratorsSectionProps> = ({
  ...
  const matchedCandle = candles.find(
    (c) => c.collaboratorId === selectedCollaborator.id || c.id === selectedCollaborator.associatedCandleId
  );
  ...
  const matchedModalCandle = modalCollaborator ? candles.find(
    (c) => c.collaboratorId === modalCollaborator.id || c.id === modalCollaborator.associatedCandleId
  ) : null;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fade-in">
          <div className="bg-[#FDFBF9] rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-[#E5E0DA] shadow-2xl p-6 sm:p-8 space-y-6 relative">
            ...
            <div className="pt-2 flex items-center justify-end gap-3">
              ...
              {onSelectCandleById && (modalCollaborator.associatedCandleId || matchedModalCandle?.id) && (
                <button
                  onClick={() => {
                    const candleId = (modalCollaborator.associatedCandleId || matchedModalCandle?.id)!;
                ...
                >
                  <span>Ver Vela Asociada ({modalCollaborator.associatedCandleName || matchedModalCandle?.name})</span>
