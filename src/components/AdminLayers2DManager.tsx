...
export const AdminLayers2DManager: React.FC<AdminLayers2DManagerProps> = ({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 1. Mostrar preview instantánea con base64
      handleUpdateLayer(targetLayer.id, { imageUrl: compressedBase64 });

      // 2. Si Supabase está configurado, subir a Supabase Storage
      if (isSupabaseConfigured()) {
        try {
          ...
          console.warn("Fallo al subir capa a Supabase Storage en AdminLayers2DManager:", cloudErr);
        }
      }

      // 3. Fallback al servidor local
      try {
