...
import { SANTUARIO_VIRGEN_LAYERS, ZORRO_BOSQUE_LAYERS, DEFAULT_SCULPTED_FIGURES } from "../data/sampleLayers2D";
...
interface StoreContextType {
  ...
  saveReviewsToStorage: (items: CandleReview[]) => void;
...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const saveStoreSettingToSupabase = (key: string, value: any) => {
    ...
    supabase
      ...
      .then(({ error }) => {
        if (error) {
          console.warn(`Aviso al sincronizar '${key}' en Supabase store_settings:`, error.message);
        }
      });
  };
  ...
  const saveCollaboratorsToStorage = (items: Collaborator[]) => {
    try {
      const serialized = JSON.stringify(items);
  ...
  const saveSculpturesToStorage = (items: SculptedFigure[]) => {
    try {
      const serialized = JSON.stringify(items);
  ...
  const saveReviewsToStorage = (items: CandleReview[]) => {
    try {
      const serialized = JSON.stringify(items);
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          return parsed.map((item: CandleProduct) => {
            if (!item.layers2D || item.layers2D.length === 0) {
  ...
  const saveCustomizerOptionsToStorage = (opts: CustomizerOptionsCatalog) => {
    try {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(serverCollabs) && serverCollabs.length > 0) {
          ...
          saveCollaboratorsToStorage(serverCollabs);
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(serverReviews) && serverReviews.length > 0) {
          ...
          saveReviewsToStorage(serverReviews);
  ...
  const updateBrandConfig = (newConfig: Partial<BrandConfig>) => {
    setBrandConfig((prev) => {
      ...
      saveStoreSettingToSupabase("brand_config", next);
      return next;
    });
  };
